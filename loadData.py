...
def load_data(file_path: str, num_data_points: int, padding_value: float=0.0) -> np.ndarray:
    ...
    data points, (which are time series with 12 channels of cepstrum coefficients), 
    pads them to the maximum sequence length (MAX_LENGTH), and returns a 3D NumPy 
    array of shape (num_data_points, MAX_LENGTH, 12).
